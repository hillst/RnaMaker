...
my $fasta = "/nfs4shares/bioinfosw/bin/fasta35";
...
#path to where our dbs are stored.
my $db_path = "/var/www/RnaMaker/amirna_dbs";
...
print STDERR "Small RNA = $sRNA\n" if (DEBUG);
print STDERR "Query name = $query_name\n" if (DEBUG);
...
print STDERR "Creating temporary file $infile\n" if (DEBUG);
...
# Run FASTA
print STDERR " Running $fasta... " if (DEBUG);
@fasta = &fasta($infile, $database);
print STDERR "done\n" if (DEBUG);

# Parse FASTA results
print STDERR " Parsing results... " if (DEBUG);
@fasta_parsed = &fasta_parser(@fasta);
print STDERR "done\n" if (DEBUG);

# Score alignments
print STDERR " Scoring alignments... " if (DEBUG);
my @targets = &bp_score(@fasta_parsed);
print STDERR "done\n" if (DEBUG);

if (@targets) {
	print STDERR " Finding additional hits... " if (DEBUG);
	...
	print STDERR " Getting target site coordinates... " if (DEBUG);
	@targets = get_coords($database, 1, @targets);
	print STDERR "done\n" if (DEBUG);
}
	
if ($opt_r) {
	print STDERR " Creating reverse database... " if (DEBUG);
	...
	# Run FASTA
	print STDERR " Running FASTA34 on reverse database... " if (DEBUG);
	@fasta = &fasta($infile, $revdb);
	print STDERR "done\n" if (DEBUG);

	# Parse FASTA results
	print STDERR " Parsing reverse results... " if (DEBUG);
	@fasta_parsed = &fasta_parser(@fasta);
	print STDERR "done\n" if (DEBUG);

	# Score alignments
	print STDERR " Scoring reverse alignments... " if (DEBUG);
	my @rev_targets = &bp_score(@fasta_parsed);
	print STDERR "done\n" if (DEBUG);
	if (@rev_targets) {
		print STDERR " Finding additional reverse targets... " if (DEBUG);
		...
		print STDERR "done\n" if (DEBUG);
		# Get target site coorinates
		print STDERR " Getting coordinates for reverse targets... " if (DEBUG);
...
if (@targets) {
	...
	# Print results, make sure to check length of inner array
	my $i = 0;
	foreach my $line (@targets) {
		$line->{'target_strand'} = ($line->{'target_strand'} == 1) ? '+' : '-';
		$line->{'homology_string'} =~ s/ /\&nbsp/g;
        $result .= "\"hit-$i\": {\n";
		$result .= "\t\"hit_accession\": \"$line->{'hit_accession'}\",\n";
		$result .= "\t\"score\":\"$line->{'score'}\",\n";
		$result .= "\t\"strand\":\"$line->{'target_start'}-$line->{'target_end'} ($line->{'target_strand'})\",\n";
		$result .= "\t\"target_seq\":\"$line->{'target_seq'}\",\n";
		$result .= "\t\"homology_string\":\"$line->{'homology_string'}\",\n";
		$result .= "\t\"miR_seq\":\"$line->{'miR_seq'}\"\n";
        $result .= "}\n,";
        $i++;
	}
    $result = substr($result,0,-1);
} else {
    $result.= "\"No results for $query_name\"}";
}
$result .= "}";
print $result;
	...
	foreach my $line (@input) {
		...
		if ($step == 0) {
			if ($line =~ /^\>{2}(.+)\(\d+ nt\)/) {
				...
				$step = 1;
				print STDERR "     HIT ACCESSION = $hit_accession\n" if (DEBUG);
			}
		} elsif ($step == 1) {
